import { AlertTriangle, Ban, CheckCircle2, CheckSquare, Clock3, ExternalLink, FolderOpen, Link2, MoreVertical, Pause, Play, Plus, Search, Square, Trash2 } from "lucide-react";
...
export function DownloadsPage({settings,filter}:{settings:AppSettings;filter:PageId}){
  ...
  useEffect(()=>{const refresh=()=>void service.browserExtensionConnected().then(setExtensionConnected).catch(()=>setExtensionConnected(false));refresh();const timer=window.setInterval(refresh,5000);return()=>window.clearInterval(timer)},[]);
  ...
  const openDetails=(event:React.MouseEvent)=>{
    const row=(event.target as HTMLElement).closest<HTMLElement>(".reference-row");
    if(!row)return;
    const index=[...row.parentElement!.querySelectorAll(".reference-row")].indexOf(row);
    const item=visible[index];
    if(item)void service.openProgressWindow(item.id);
  };
  
  return <section className="downloads-workspace" onDoubleClick={openDetails}>
   <div className="reference-toolbar"><button className="new-download-button" onClick={()=>setComposer(value=>!value)}><Plus/>Novo</button><button className="toolbar-action" disabled={!selected.size} onClick={() => {
     if (window.confirm(selected.size === 1 ? "Tem certeza que deseja excluir o download selecionado?" : `Tem certeza que deseja excluir os ${selected.size} downloads selecionados?`)) {
       void remove([...selected]).then(() => setSelected(new Set()));
     }
   }}><Trash2/>Excluir</button><button className="toolbar-action" disabled={picked.length!==1||picked[0].status!=="completed"} onClick={()=>void service.openFile(picked[0].finalPath)}><ExternalLink/>Abrir</button><button className="toolbar-action" disabled={picked.length!==1} onClick={()=>void service.revealInFolder(picked[0].finalPath)}><FolderOpen/>Pasta</button><i className="toolbar-divider"/><label className="toolbar-search"><Search/><input value={search} onChange={event=>setSearch(event.target.value)} placeholder="Buscar"/></label><button className="toolbar-more"><MoreVertical/></button></div>
   ...
   <footer className="downloads-statusbar"><div className="extension-connection"><i className={extensionConnected?"connected":""}/><span>Extensão {extensionConnected?"conectada":"desconectada"}</span></div><div><Clock3/><span>{selected.size?`${selected.size} selecionado(s)`:"Fila pronta"}</span></div><div className="statusbar-ok"><CheckCircle2/><span>{downloads.some(item=>item.status==="downloading")?"Baixando":"Tudo em dia"}</span></div></footer>
         ...
         onClick={() => {
           ...
           if (val !== null) {
             ...
             if (Number.isFinite(parsed) && parsed >= 0) {
               void service.updateSpeedLimit(contextMenu.downloadId, Math.round(parsed * 1024 * 1024));
             }
           }
           setContextMenu(null);
         }}
